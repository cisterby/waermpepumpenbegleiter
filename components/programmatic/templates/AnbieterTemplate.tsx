...
// "waermepumpe-anbieter" — transactional
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Lokal vs. bundesweit: Welcher Anbieter in {stadt}?', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              In <strong>{city.name}</strong> gibt es lokale Meisterbetriebe und bundesweite Anbieter wie Thermondo oder Enpal. Der entscheidende Unterschied: Lokale Betriebe kennen die regionalen Gegebenheiten, sind schneller vor Ort und bieten flexiblere Lösungen.
            </p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-wp-bg border-b border-wp-border">
                    {["",`Lokal ${city.name} ⭐`,"Thermondo / Enpal"].map((h,i) => (
                      <th key={i} className={`px-4 py-3 text-left text-xs font-bold uppercase tracking-wider ${i===1?"text-wp-green":"text-wp-text3"}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    {feat:"Vor Ort bei Störung",lok:"✅ Innerhalb 24h",bund:"⚠️ 48–96h Wartezeit"},
                    {feat:"Regionale Kenntnisse",lok:"✅ "+city.name+" bekannt",bund:"❌ Subunternehmer"},
                    {feat:"Flexibilität",lok:"✅ Individuelle Lösung",bund:"⚠️ Standardlösungen"},
                    {feat:"Preis",lok:"Verhandelbar",bund:"Oft günstiger"},
                    {feat:"KfW-Erfahrung",lok:"✅ Alle unsere Betriebe",bund:"✅ Ja"},
                    {feat:"Mehrere Angebote",lok:"✅ 3 Betriebe vergleichen",bund:"❌ Nur 1 Angebot"},
                  ].map((r,i) => (
                    <tr key={i} className="border-b border-wp-border last:border-0">
                      ...
                      <td className="px-4 py-3 text-xs bg-wp-greenxlt text-wp-green font-medium">{r.lok}</td>
                      <td className="px-4 py-3 text-xs text-wp-text3">{r.bund}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
