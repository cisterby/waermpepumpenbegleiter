...
// "waermepumpe-foerderung" — info_commercial
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Wie hoch ist die Förderung für Wärmepumpen in {stadt}?', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              In <strong>{city.name}</strong> ({city.bundesland}) beträgt die KfW-Bundesförderung bis zu <strong>70%</strong> der förderfähigen Kosten (max. €30.000 = max. €21.000 Zuschuss). Der Zuschuss ist nicht rückzahlbar.
            </p>
            <div className="space-y-3 mb-6">
              {[
                {pct:30,label:"Grundförderung",info:"Immer · für alle Eigennutzer und Vermieter",active:true},
                {pct:20,label:"Klima-Speed-Bonus",info:"Bei Ersatz einer fossilen Heizung · nur Eigennutzer",active:true},
                {pct:30,label:"Einkommens-Bonus",info:"Haushaltseinkommen unter €40.000 netto/Jahr",active:false},
                {pct:5, label:"Kältemittel-Bonus",info:"Natürliches Kältemittel R290 oder Erdwärme",active:false},
              ].map((b,i) => (
                <div key={i} className={`p-4 rounded-xl border ${b.active?"border-wp-green bg-wp-greenlt":"border-wp-border bg-white"}`}>
                  <div className="flex justify-between items-center mb-2">
                    <p className={`font-heading font-semibold text-sm ${b.active?"text-wp-green":"text-wp-text2"}`}>{b.label}</p>
                    <span className="font-mono font-bold text-wp-amber">+{b.pct}%</span>
                  </div>
                  <div className="h-1.5 bg-wp-border rounded-full overflow-hidden mb-2">
                    <div className="h-full bg-wp-green rounded-full" style={{width:`${b.pct/70*100}%`}} />
                  </div>
                  <p className="text-wp-text3 text-xs">{b.info}</p>
                </div>
              ))}
            </div>
            <div className="bg-wp-dark rounded-xl p-4 flex justify-between items-center mb-4">
              <span className="text-white font-semibold">Typisch (Eigennutzer, fossile Heizung)</span>
            ...
            {city.bundeslandFoerderung && (
              <div className="bg-white border border-wp-border rounded-xl p-4">
                <p className="font-heading font-semibold text-wp-green mb-1">+ {city.bundesland}: {city.bundeslandFoerderung}</p>
                <p className="text-wp-text2 text-sm">{city.bundeslandFoerderungBetrag}</p>
              </div>
            )}
