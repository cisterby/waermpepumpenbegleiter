          ...
          <div className="grid sm:grid-cols-2 gap-8">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {nearby.map(n => (
                  <Link key={n.slug} href={'/' + keyword.slug + '/' + n.slug} className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-sm text-[#4A6358] hover:text-[#1A4731] hover:border-[#1A4731] transition-colors">
                    {n.name}
                  </Link>
                ))}
              </div>
