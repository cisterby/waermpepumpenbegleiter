...
// "waermepumpe-installateur" — transactional
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Worauf achten bei der Wahl des Installateurs in {stadt}?', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              Die Verbraucherzentrale hat bei 160 geprüften WP-Angeboten festgestellt: Lokale Betriebe lassen oft Positionen weg. Unsere Partnerbetriebe in <strong>{city.name}</strong> erfüllen alle diese Kriterien:
            </p>
            <div className="space-y-3 mb-6">
              {[
                {icon:"🏆",title:"HWK-eingetragener Meisterbetrieb",text:"Grundvoraussetzung für KfW-Förderung — ohne eingetragenen Fachbetrieb kein Antrag möglich."},
                {icon:"📋",title:"Mindestens 5 WP-Installationen nachweisbar",text:"Erfahrung mit der Dimensionierung, Hydraulik und KfW-Antragsprozess."},
                {icon:"🛡️",title:"Gültige Haftpflichtversicherung",text:"Schützt Sie bei Schäden während der Installation."},
                {icon:"📄",title:"Vollständiges Angebot mit allen Positionen",text:"Gerät, Montage, Hydraulik, Elektrik, Förderberatung — alles ausgewiesen."},
                {icon:"📍",title:"Lokal in {city.name} ansässig",text:"Bei Störungen schnell vor Ort — im Gegensatz zu bundesweiten Anbietern."},
              ].map((item,i) => (
                <div key={i} className="flex gap-4 p-4 bg-white rounded-xl border border-wp-border shadow-wp-sm">
                  <span className="text-2xl shrink-0">{item.icon}</span>
                  <div>
                    <p className="font-heading font-semibold text-wp-text mb-1">{item.title}</p>
                    <p className="text-wp-text2 text-sm leading-relaxed">{item.text.replace("{city.name}",city.name)}</p>
                  </div>
                </div>
              ))}
            </div>
