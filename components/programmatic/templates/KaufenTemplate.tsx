...
export default function KaufenTemplate({ city, keyword, calc, foerd, jaz, nearby, h1 }: CityPageRouterProps) {
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10 w-full py-20">
          ...
          <div className="fade-in-up">
            ...
            <h1 className="font-extrabold text-white leading-tight mb-4" style={{ fontSize: 'clamp(30px,4.5vw,56px)' }}>
              {h1}
            </h1>
              {/* Ultra-lokale Fakten */}
            ...
            <div className="flex gap-3 flex-wrap">
              <a href="#angebot" className="inline-flex items-center gap-2 px-7 py-4 bg-[#1A4731] text-white rounded-xl font-bold text-sm hover:bg-green-800 transition-all hover:-translate-y-0.5 shadow-xl">
  ...
  return (
              <>
                <h2 className="font-bold text-[#1C2B2B] mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
                ...
                <div className="space-y-3 mb-6">
                  <p className="font-semibold text-[#1C2B2B]">5 Schritte zur richtigen WP in {city.name}:</p>
                  {stepVariants[v].map((s, i) => (
                    <div key={i} className="flex gap-4 p-4 bg-white rounded-xl border border-gray-200 shadow-md">
                      <div className="w-8 h-8 bg-[#1A4731] rounded-full flex items-center justify-center font-bold text-white text-sm shrink-0">{s.n}</div>
                      <div>
                        <p className="font-semibold text-[#1C2B2B] mb-1">{s.title}</p>
