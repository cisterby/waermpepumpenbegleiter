...
// "luftwaermepumpe" — info_commercial
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Luft-Luft oder Luft-Wasser in {stadt}?', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              Beide nutzen die Außenluft als Wärmequelle — der Unterschied liegt in der Wärmeabgabe. In <strong>{city.name}</strong> mit {city.heizgradtage} Heizgradtagen ist die <strong>Luft-Wasser-WP</strong> (Heizung + Warmwasser) die richtige Wahl für Einfamilienhäuser.
            </p>
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              {[
                {typ:"Luft-Wasser",icon:"🏠",empfohlen:true,text:"Ersetzt die komplette Heizung. Heizt über das Wassersystem (Heizkörper oder FBH) und bereitet Warmwasser auf. In "+city.name+" für EFH die Standardlösung.",pros:["Ersetzt Gas-/Ölheizung komplett","Warmwasser inklusive","KfW-förderungsfähig","JAZ "+String(jaz)+" in "+city.name],kosten:"€18.000–28.000"},
                {typ:"Luft-Luft",icon:"💨",empfohlen:false,text:"Heizt nur über Luft (wie Klimaanlage). Kein Warmwasser. In Deutschland selten — kein Heizkörpersystem, kein KfW-Zuschuss in gleicher Höhe.",pros:["Günstiger (€5.000–12.000)","Auch zum Kühlen nutzbar","Schnelle Installation"],kosten:"€5.000–12.000"},
              ].map((t,i) => (
                <div key={i} className={`rounded-xl border-2 p-5 shadow-wp-sm ${t.empfohlen?"border-wp-green bg-wp-greenlt":"border-wp-border bg-white"}`}>
                  {t.empfohlen && <span className="inline-block bg-wp-green text-white text-xs font-bold px-2 py-0.5 rounded-full mb-3">Empfohlen für {city.name}</span>}
                  <div className="text-3xl mb-2">{t.icon}</div>
                  <h3 className="font-heading font-semibold text-wp-text mb-1">{t.typ}-Wärmepumpe</h3>
                  <p className="font-mono font-bold text-wp-amber text-sm mb-3">{t.kosten}</p>
                  <p className="text-wp-text2 text-xs leading-relaxed mb-3">{t.text}</p>
                  {t.pros.map(p => <div key={p} className="flex gap-2 text-xs text-wp-text2 mb-1"><span className="text-wp-green">✓</span>{p}</div>)}
                </div>
              ))}
            </div>
