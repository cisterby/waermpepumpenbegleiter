...
export default function StromverbrauchTemplate({ city, keyword, calc, foerd, jaz, nearby, h1 }: CityPageRouterProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] font-sans">
              ...
              {TARIF_VERGLEICH.map((t, i) => (
                <div key={i} className={`p-4 rounded-xl border ${i === 2 ? 'bg-[#F2FAF5] border-gray-200l' : 'bg-white border-gray-200'}`}>
                  <div className="flex justify-between items-start gap-3">
                    <div>
                      <div className="font-bold text-[#1C2B2B] text-sm">{t.tarif}</div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {OPTIMIERUNG.map((o, i) => (
                <div key={i} className="p-4 bg-white border border-gray-200 rounded-xl">
                  ...
                  <div className="font-semibold text-[#1C2B2B] text-sm mb-1">{o.titel}</div>
      ...
      {city.strompreis >= 31.5 && (
        <div className="max-w-3xl mx-auto px-6 pb-6">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            <span className="text-xl shrink-0">⚡</span>
            <div>
              <p className="font-bold text-amber-800 text-sm mb-1">{city.name}: {city.strompreis} ct/kWh — WP-Sondertarif prüfen</p>
              <p className="text-amber-700 text-xs leading-relaxed">Bei {city.strompreis} ct/kWh lohnt sich der WP-Sondertarif beim lokalen Netzbetreiber besonders. Mit Sondertarif (ca. 22–26 ct/kWh) sinkt der Jahresstromverbrauch der WP von {fmtEuro(calc.wpKosten)} auf ca. {fmtEuro(Math.round(calc.wpKosten * 0.75))} — ein Unterschied von {fmtEuro(Math.round(calc.wpKosten * 0.25))}/Jahr.</p>
            </div>
          </div>
        </div>
      )}
