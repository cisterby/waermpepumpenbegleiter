      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
            ...
            <div className="overflow-x-auto rounded-2xl border border-wp-border shadow-wp-sm bg-white mb-6">
              <table className="w-full text-sm min-w-[500px]">
                <thead>
                  <tr className="bg-wp-bg border-b border-wp-border">
                    {["Heizung","Eigenanteil","GEG-konform","Betriebskosten/J.","Empfehlung"].map(h => (
                ...
                <tbody>
                  {[
                    {h:"Wärmepumpe ⭐",ei:fmtEuro(foerd.eigenanteil),geg:"✅ Ja",kosten:fmtEuro(calc.wpKosten)+"/J.",emp:"Beste Wahl",highlight:true},
                    {h:"Gas-Brennwert",ei:"€4.000–8.000",geg:"⚠️ Nur mit Solar",kosten:fmtEuro(calc.altKosten)+"/J.",emp:"Risiko CO₂-Preis",highlight:false},
                    {h:"Pellets",ei:"€12.000–20.000",geg:"✅ Ja",kosten:"€1.200–1.800/J.",emp:"Wartungsintensiv",highlight:false},
                    {h:"Fernwärme",ei:"€3.000–8.000",geg:"Abhängig",kosten:"Variiert stark",emp:"Wenn vorhanden",highlight:false},
                  ].map((r,i) => (
                    <tr key={i} className={`border-b border-wp-border last:border-0 ${r.highlight?"bg-wp-greenxlt":""}`}>
                      <td className="px-4 py-3 font-semibold text-wp-text">{r.h}</td>
                      <td className="px-4 py-3 font-mono text-wp-text2">{r.ei}</td>
                      <td className="px-4 py-3">{r.geg}</td>
                      <td className={`px-4 py-3 font-mono ${r.highlight?"text-wp-green font-bold":"text-wp-text2"}`}>{r.kosten}</td>
                      <td className={`px-4 py-3 text-xs font-semibold ${r.highlight?"text-wp-green":"text-wp-text3"}`}>{r.emp}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
