...
// "waermepumpe-fachbetrieb" — transactional
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Was macht einen WP-Fachbetrieb in {stadt} aus?', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              Nicht jeder SHK-Betrieb ist ein WP-Fachbetrieb. Für die KfW-Förderung brauchen Sie einen <strong>registrierten Fachbetrieb</strong> (Energieeffizienz-Experte oder eingetragenes Fachunternehmen). Alle unsere Partnerbetriebe in {city.name} erfüllen das.
            </p>
            <div className="bg-white rounded-2xl border border-wp-border p-6 shadow-wp-sm mb-6">
              <p className="font-heading font-semibold text-wp-text mb-4">Zertifizierungscheck für {city.name}:</p>
              <div className="space-y-3">
                {[
                  {check:"Handwerkskammer-Eintragung",required:true,info:"Pflicht — ohne HWK-Eintrag keine KfW-Förderung"},
                  {check:"Fachunternehmen BEG-Programm",required:true,info:"Muss im KfW-Portal als Lieferanten- und Leistungserbringer registriert sein"},
                  {check:"Mindestens 5 WP-Referenzen",required:false,info:"Empfohlen — Erfahrung mit Dimensionierung und Einstellung"},
                  {check:"BWP-Mitglied",required:false,info:"Bundesverband Wärmepumpe — zusätzliche Qualitätssicherung"},
                  {check:"Wartungsvertrag im Angebot",required:false,info:"Sichert langfristig optimale Effizienz"},
                ].map((s,i) => (
                  <div key={i} className={`flex items-start gap-3 p-3 rounded-lg ${s.required?"bg-wp-greenlt":"bg-wp-bg"}`}>
                    <span className={`text-sm font-bold shrink-0 ${s.required?"text-wp-green":"text-wp-text3"}`}>{s.required?"✅":"⭐"}</span>
                    <div>
                      <p className={`font-semibold text-sm ${s.required?"text-wp-green":"text-wp-text"}`}>{s.check} {s.required?"(Pflicht)":"(Empfohlen)"}</p>
                      <p className="text-wp-text3 text-xs mt-0.5">{s.info}</p>
                    </div>
                  </div>
                ))}
              </div>
