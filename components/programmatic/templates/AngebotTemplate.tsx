...
// "waermepumpe-angebot" — transactional
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Wärmepumpe Angebot {stadt} — Worauf Sie achten müssen', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              Holen Sie mindestens 3 Angebote ein — die Verbraucherzentrale empfiehlt das ausdrücklich. Preisunterschiede von <strong>20–40%</strong> bei gleicher Leistung sind in {city.name} keine Seltenheit. Wir vermitteln kostenlos an geprüfte lokale Betriebe.
            </p>
            <div className="bg-white rounded-2xl border border-wp-border p-6 shadow-wp-sm mb-6">
              <p className="font-heading font-semibold text-wp-text mb-4">Was ein gutes Angebot enthält:</p>
              <div className="space-y-2">
                {[
                  {icon:"✅",text:"Heizlastberechnung nach DIN EN 12831 (wichtig für korrekte Dimensionierung)"},
                  {icon:"✅",text:"Alle Positionen einzeln ausgewiesen: Gerät, Montage, Hydraulik, Elektrik"},
                  {icon:"✅",text:"Hydraulischer Abgleich inklusive (KfW-Pflicht)"},
                  {icon:"✅",text:"KfW-Förderantrag im Angebot berücksichtigt"},
                  {icon:"✅",text:"Garantie und Wartungsvertrag-Option"},
                  {icon:"❌",text:"Vorsicht bei: Pauschalpreisen ohne Aufschlüsselung, Druck zum sofortigen Abschluss"},
                ].map((s,i) => (
                  <div key={i} className={`flex gap-3 p-3 rounded-lg text-sm ${s.icon==="❌"?"bg-red-50 text-red-800":"bg-wp-bg text-wp-text2"}`}>
                    <span className="shrink-0">{s.icon}</span>{s.text}
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-wp-dark rounded-xl p-4 flex justify-between items-center">
              <div>
                <p className="text-white font-semibold">Ihr Eigenanteil nach Förderung</p>
                <p className="text-white/50 text-xs">bei typischem EFH in {city.name}</p>
              </div>
              <p className="font-mono font-bold text-wp-amber text-2xl">{fmtEuro(foerd.eigenanteil)}</p>
            </div>
