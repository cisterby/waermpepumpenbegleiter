...
// "waermepumpe-oder-gas" — info_commercial
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              Kurzantwort: Mit der KfW-Förderung ist die Wärmepumpe für Eigennutzer in <strong>{city.name}</strong> langfristig günstiger — besonders wenn man den steigenden CO₂-Preis auf Erdgas einrechnet (2026: 55 €/t, 2030: voraussichtlich 100 €/t).
            </p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-wp-bg border-b border-wp-border">
                    {["",`Wärmepumpe ⭐`,"Gas-Brennwert"].map((h,i) => (
                      <th key={i} className={`px-4 py-3 text-left text-xs font-bold uppercase ${i===1?"text-wp-green":"text-wp-text3"}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    {feat:"Investition",wp:fmtEuro(foerd.eigenanteil)+" (nach KfW)",gas:"€4.000–8.000"},
                    {feat:"Betriebskosten/Jahr",wp:fmtEuro(calc.wpKosten),gas:fmtEuro(calc.altKosten)},
                    {feat:"CO₂-Preis 2030 (Aufschlag)",wp:"Nicht betroffen",gas:"+€200–400/Jahr"},
                    {feat:"GEG-konform ab 2026",wp:"✅ Ja",gas:"⚠️ Nur mit Solar-Anteil"},
                    {feat:"Lebensdauer",wp:"20–25 Jahre",gas:"15–20 Jahre"},
                    {feat:"Betriebskosten 2030",wp:fmtEuro(calc.wpKosten)+" (≈ gleich)",gas:fmtEuro(calc.altKosten+350)+" (+CO₂)"},
                  ].map((r,i) => (
                    <tr key={i} className="border-b border-wp-border last:border-0">
                      <td className="px-4 py-3 font-semibold text-wp-text2 text-xs">{r.feat}</td>
                      <td className="px-4 py-3 text-xs text-wp-green font-medium bg-wp-greenxlt">{r.wp}</td>
                      <td className="px-4 py-3 text-xs text-wp-text3">{r.gas}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="bg-wp-greenlt border border-wp-green3/40 rounded-xl p-4">
              <p className="font-heading font-semibold text-wp-green mb-2">Fazit für {city.name}</p>
              <p className="text-wp-text2 text-sm leading-relaxed">Mit {city.strompreis} ct/kWh Strom und {city.gaspreis} ct/kWh Gas sparen Sie mit einer WP bereits heute {fmtEuro(calc.ersparnis)}/Jahr. Bis 2030 wird der Abstand durch den CO₂-Preis noch größer.</p>
            </div>
