...
export default function PreiseTemplate({ city, keyword, calc, foerd, jaz, nearby, h1 }: CityPageRouterProps) {
  ...
  const intros = [
    `WP-Preise ${city.name} 2026: Luft-WP €${preisRange.lwpVon.toLocaleString('de-DE')}–€${preisRange.lwpBis.toLocaleString('de-DE')}, nach ${foerd.gesamtSatz}% KfW (${fmtEuro(foerd.zuschuss)}) noch €${eigenanteilMin.toLocaleString('de-DE')}–€${eigenanteilMax.toLocaleString('de-DE')} Eigenanteil. Versteckte Kosten (Elektro, Puffer, Entsorgung) machen oft 15–25% der Gesamtrechnung aus.`,
    ...
    `Preisvergleich ${city.name} (${city.bundesland}): Luft-WP ab €${preisRange.lwpVon.toLocaleString('de-DE')}, Sole-WP €${preisRange.erdVon.toLocaleString('de-DE')}+. Nach ${foerd.gesamtSatz}% KfW = Eigenanteil ${fmtEuro(foerd.eigenanteil)}. Achtung: Angebote ohne Heizlastberechnung immer ablehnen.`,
    `Was kostet eine WP in ${city.name}? Gerät + Montage + Nebenkosten (Elektro, Puffer, Entsorgung) = €${preisRange.lwpVon.toLocaleString('de-DE')}–€${preisRange.lwpBis.toLocaleString('de-DE')} vor Förderung. ${foerd.gesamtSatz}% KfW deckt ${fmtEuro(foerd.zuschuss)} ab. Eigenanteil: ${fmtEuro(foerd.eigenanteil)}.`,
  ];
        ...
        <div className="lg:col-span-2 space-y-14">
          ...
          <div>
            ...
            <p className="text-[#4A6358] text-base leading-relaxed">
              Eine Luft-Wasser-WP in <strong>{city.name}</strong> kostet inklusive Montage und Nebenkosten €{preisRange.lwpVon.toLocaleString('de-DE')}–€{preisRange.lwpBis.toLocaleString('de-DE')}. Nach {foerd.gesamtSatz}% KfW-Förderung ({fmtEuro(foerd.zuschuss)}) verbleibt ein Eigenanteil von {fmtEuro(foerd.eigenanteil)}. Sole-WP (Erdwärme) kostet €{preisRange.erdVon.toLocaleString('de-DE')}–€{preisRange.erdBis.toLocaleString('de-DE')}, hat aber höhere Effizienz (JAZ {(jaz + 1.0).toFixed(1)}) und immer +5% KfW-Bonus.
            </p>
          </div>
            ...
            <div className="space-y-3">
              {PREIS_FAKTOREN.map((f,i)=>(
                <div key={i} className="flex gap-3 p-4 bg-white border border-gray-200 rounded-xl">
                  ...
                  <div><div className="font-bold text-[#1C2B2B] text-sm mb-1">{f.faktor}</div>
          ...
          <div className="p-6 bg-[#F2FAF5] border border-gray-200l rounded-2xl">
            <h2 className="font-bold text-[#1C2B2B] text-xl mb-4">{h2s.kosten}</h2>
            <p className="text-[#4A6358] text-base leading-relaxed mb-4">{si.kosten}</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              {[
                [`€${preisRange.lwpVon.toLocaleString('de-DE')}–€${preisRange.lwpBis.toLocaleString('de-DE')}`,'Luft-WP gesamt'],
