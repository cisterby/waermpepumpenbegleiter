              ...
              {MONTAGE_ABLAUF.map((tag, i) => (
                <div key={i} className="bg-white border border-gray-200 rounded-xl p-5 shadow-md">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="font-bold text-[#1C2B2B]">{tag.tag}</span>
                  </div>
            ...
            <div className="space-y-3">
              {MONTAGE_FEHLER.map((f, i) => (
                <div key={i} className="p-4 bg-white border border-gray-200 rounded-xl">
                  <div className="font-semibold text-[#1C2B2B] text-sm mb-1">❌ {f.fehler}</div>
      ...
      {city.avgTemp >= 11.0 && (
        <div className="max-w-3xl mx-auto px-6 pb-6">
          <div className="bg-[#E8F5EE] border border-[#3DA16A]/30 rounded-xl p-4 flex items-start gap-3">
            <span className="text-xl shrink-0">✅</span>
            <div>
              <p className="font-bold text-[#1A4731] text-sm mb-1">{city.name}: {city.avgTemp}°C — Günstiges Klima für Wärmepumpen</p>
              <p className="text-[#4A6358] text-xs leading-relaxed">Mit {city.avgTemp}°C Jahresmittel erreicht eine Luft-WP in {city.name} JAZ {jaz}. Das senkt die Betriebskosten auf {fmtEuro(calc.wpKosten)}/Jahr und macht die Investition besonders wirtschaftlich.</p>
            </div>
          </div>
        </div>
      )}
