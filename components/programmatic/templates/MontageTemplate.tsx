...
// "waermepumpe-montage" — transactional
'use client';
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-14 grid lg:grid-cols-[1fr_380px] gap-12 items-start">
        <div>
          ...
          <h2 className="font-heading font-bold text-wp-text mb-5" style={{ fontSize: 'clamp(22px,2.5vw,36px)' }}>
            {fillTemplate('Wo kann ich die Wärmepumpe in {stadt} aufstellen?', city, jaz)}
          </h2>
          <p className="text-wp-text2 text-base leading-relaxed mb-5">
              Die häufigste Frage bei der Montage: Wohin mit der Außeneinheit? In <strong>{city.bundesland}</strong> gilt in der Regel ein Mindestabstand von 3 Metern zur Grundstücksgrenze. Ihr lokaler Fachbetrieb in {city.name} kennt die genauen Auflagen.
            </p>
            <div className="space-y-3 mb-6">
              {[
                {title:"Außenaufstellung (Standardfall)",text:"Luft-WP wird auf einem Betonsockel oder Gummidämpfern neben dem Haus aufgestellt. Kosten: €300–600. Platzbedarf: ca. 1m² + Servicezugang.",icon:"🏡",empf:true},
                {title:"Innenaufstellung (Keller/Garage)",text:"WP steht im Gebäude, Luft wird über Kanäle zu- und abgeführt. Aufwendiger, aber kein Lärm außen. Kosten: €800–1.500 mehr.",icon:"🏠",empf:false},
                {title:"Dachaufstellung",text:"Bei beengten Verhältnissen möglich. Statik prüfen lassen. Gerüst nötig. Selten in "+city.name+".",icon:"🏗️",empf:false},
                {title:"Lärmschutzmaßnahmen",text:"Antivibrationsmatten (€100–200), schallabsorbierender Sichtschutz (€300–800), optimale Ausrichtung. Lärmpegel: 45–55 dB auf 1m.",icon:"🔇",empf:false},
              ].map((s,i) => (
                <div key={i} className={`flex gap-4 p-4 rounded-xl border ${s.empf?"border-wp-green bg-wp-greenlt":"border-wp-border bg-white"} shadow-wp-sm`}>
                  <span className="text-2xl shrink-0">{s.icon}</span>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-heading font-semibold text-wp-text">{s.title}</p>
                      {s.empf && <span className="bg-wp-green text-white text-xs font-bold px-2 py-0.5 rounded-full">Standardfall</span>}
                    </div>
