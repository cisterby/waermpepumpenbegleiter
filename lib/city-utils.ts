...
export function buildBreadcrumbSchema(
  ...
  return {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: keywordLabel, item: `https://xn--wrmepumpenbegleiter-gwb.de/${keywordSlug}` },
      { '@type': 'ListItem', position: 3, name: city.name,    item: `https://xn--wrmepumpenbegleiter-gwb.de/${keywordSlug}/${city.slug}` },
    ],
  }
}
...
export function buildLocalBusinessSchema(keywordSlug: string, city: City) {
  return {
    ...
    sameAs: [
      `https://xn--wrmepumpenbegleiter-gwb.de/${keywordSlug}/${city.slug}`,
    ],
    serviceArea: {
      '@type': 'GeoCircle',
